...
sti_loop:
	...
	bnez $t6, sti_overflow     # Vérifier overflow multiplication
	...
	blt $t5, $t6, sti_overflow # Vérifier overflow addition (si le chiffre après addition est plus faible qu'avant on a fait un overflow)
...
csto_erreur_overflow:
	li $v0, 4			   #
	la $a0, error_overflow # Charger adresse du message d'erreur
	syscall                #
	li $v0, 10             #
	syscall                #
...
add_string:
	...
	beqz $v0, add_string_error_not_int # Vérifier si la première chaine est un entier
	...
	beqz $v0, add_string_error_not_int # Vérifier si la deuxième chaine est un entier
...
sub_string:
	...
	move $t1, $a1  					   # Faire une copie de la deuxième chaine
	jal is_integer 					   # Vérifier si la première chaine est un entier
	beqz $v0, sub_string_error_not_int # Vérifier si la première chaine est un entier
	jal string_to_int                  # Convertir la première chaine en nombre
	move $t2, $v0                      # Enregistrer le résultat dans $t2
	jal count_char                     # Déterminer la taille de la première chaine de caractères
	move $t4, $v0                      # Enregistrer le résultat dans $t4
	...
	move $a0, $a1					   # Placer la deuxième chaine dans $a0
	jal is_integer                     # Vérifier si la deuxième chaine est un entier
	beqz $v0, sub_string_error_not_int # Vérifier si la deuxième chaine est un entier
	jal string_to_int                  # Convertir la deuxième chaine en nombre
	move $t3, $v0                      # Enregistrer le résultat dans $t3
	jal count_char                     # Déterminer la taille de la deuxième chaine de caractères
	move $t5, $v0                      # Enregistrer le résultat dans $t5
...
mult_string:
	...
	jal string_to_int #on converti l'entier en nombre
	move $t2, $v0
	move $a0, $a1
	jal is_integer # on test si l'arg 1 est bien un entier 
	beqz $v0, mult_string_error_not_int
...
mod_next_1:
	...
	jal int_to_string
	j mod_string_exit
mod_string_exit: # Faire label pour entrer et sortir de fonction ??
	lw   $ra, 40($sp) # Restaurer $t9
	lw   $t9, 36($sp) # Restaurer $t9
	lw   $t8, 32($sp) # Restaurer $t8
	lw   $t7, 28($sp) # Restaurer $t7
